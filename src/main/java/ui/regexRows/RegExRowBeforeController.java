...
public class RegExRowBeforeController extends RegexRowController<RegExRowBeforeView>
{
   public RegExRowBeforeController(BiConsumer<Match, Condition> matchConsumer)
   {
      super(new RegExRowBeforeView(), matchConsumer);
   }
   ...
   public Supplier<Optional<Match>> getMatchSupplier()
   {
      return () -> {
         ...
         {
            Match match = new Match(MatchType.BEFORE_TEXT);
            match.setText(text);
